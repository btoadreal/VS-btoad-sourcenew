...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (ClientPrefs.data.cacheDeath)
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				openSubState(new GameOverSubstate(BF_X, BF_Y, camFollow.x, camFollow.y));
	...
	public function endSong()
	{
		...
		{
			var achieve:Array<String> = checkForAchievements(['${WeekData.getWeekFileName()}_nomiss', 'ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger']);

			if(achieve != null && achieve.length > 0)
			{
				onNextAchievement(achieve);
				return false;
			}
		}
		#end
	...
	function startAchievement(achievement:String) {
		achievementObj = new AchievementPopup(achievement, camOther);
		achievementObj.onFinish = function() {achievementObj = null;};
		add(achievementObj);
		trace('Giving Achievement "$achievement"');
	}
	
	function onNextAchievement(list:Array<String>):Void
	{
		var achievement:String = list[0];

		if (list.length > 0) {
			Achievements.unlock(achievement);
			achievementObj = new AchievementPopup(achievement, camOther);
			list.shift();
			achievementObj.onFinish = onNextAchievement.bind(list);
			add(achievementObj);
			trace('Giving Achievement "$achievement"');
		}
		else {
	...
	private function keysCheck():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			if (holdArray.contains(true) && !endingSong) {
				#if ACHIEVEMENTS_ALLOWED
				if (canUnlockAcv('oversinging')) {
					startAchievement('oversinging');
				}
				#end
			}
			else if (boyfriend.animation.curAnim != null && boyfriend.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * boyfriend.singDuration && boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.animation.curAnim.name.endsWith('miss'))
	...
	#if ACHIEVEMENTS_ALLOWED
	private function checkForAchievements(achievesToCheck:Array<String> = null):Array<String>
	{
		if (chartingMode) // Stops if conditions weren't met (aka. if player was cheating)
			return [];

		var unlockedAchv:Array<String> = []; // Storing all new unlocked achievements in an array.

		for (achievementName in achievesToCheck) {
			if (canUnlockAcv(achievementName))
				unlockedAchv.push(achievementName);
		}
		return unlockedAchv;
	}

	function canUnlockAcv(achievementName:String):Bool
	{	
		if(Achievements.isUnlocked(achievementName)) // Already unlocked? There's no point in doing it again then.
			return false;

		switch(achievementName) // Check for achievements that don't require skill first.
		{
			case 'toastie': // HAHA! BAD PC!!
				return (!ClientPrefs.data.shaders && ClientPrefs.data.lowQuality && !ClientPrefs.data.antialiasing);
		}

		if (practiceMode) // Cheating? Stop the code immediately.
			return false;

		if (achievementName.endsWith('nomiss'))
		{
			// Additional if check to identify if the achievement is granted for not missing in hard mode.
			return (isStoryMode && campaignMisses + songMisses < 1 && Difficulty.getString().toUpperCase() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty);
		}
		else
		{
			// Check for any other achievement that doesn't require different code structure.

			switch(achievementName)
			{
				case 'ur_bad': // BAD ACCURACY! LOL SO BAD!!
					return ratingPercent < 0.2;
				case 'ur_good': // GOOD ACCURACY! PRO!!
					return ratingPercent >= 1;
				case 'oversinging': // BEEP BOP BAP - Boyfriend, maybe.
					return boyfriend.holdTimer >= 10;
				case 'hype': // LET'S GOOOO!!!
					return !boyfriendIdled;
				case 'two_keys': // He took "Left, Right!" seriously.
					return (keysPressed.length <= 2);
				case 'debugger': // wait how'd you find this..
					return Paths.formatToSongPath(SONG.song) == 'test';
				default: // who are you looking for bruh!
					return false;
			}
		}
	}
